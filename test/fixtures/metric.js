...
module.exports = {
  ...
  all: metrics,
  findUuid: uuid => {
    let types = metrics.filter(m => m.agentId === agentFixture.findByUuid(uuid).id).map((item, i) => item.type)
    return types.filter((elem, pos) => types.indexOf(elem) === pos)
  },
  findTypeAgentUuid: (type, uuid) => metrics.filter(m => m.type === type && m.agentId === agentFixture.findByUuid(uuid).id).sort((a, b) => new Date(b.date) - new Date(a.date))
}
